...
class _LanguageSelectionViewState extends State<LanguageSelectionView> {
  ...
  void initState() {
    ...
    if (cubit.state.status == LanguageSelectionStatus.initial) {
      // Fire and forget - trigger initial load
      // ignore: discarded_futures
      cubit.loadLanguages();
    }
  }
    ...
    return Scaffold(
      ...
      body: BlocConsumer<LanguageSelectionCubit, LanguageSelectionState>(
        ...
        builder: (context, state) {
          if (state.status == LanguageSelectionStatus.initial ||
              state.status == LanguageSelectionStatus.loading) {
