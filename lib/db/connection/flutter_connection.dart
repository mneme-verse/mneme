...
QueryExecutor openConnection({
  ...
  return LazyDatabase(() async {
    ...
    final effectiveOptions = DriftNativeOptions(
      // Always enforce the database path to point to the copied/local file
      databasePath: () async => file.path,
      shareAcrossIsolates: options?.shareAcrossIsolates ?? true,
      isolateDebugLog: options?.isolateDebugLog ?? false,
      tempDirectoryPath: options?.tempDirectoryPath,
      setup: options?.setup,
    );
